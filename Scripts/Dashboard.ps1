# =============================================================================
# AutoTask Dashboard V8.4 - 邏輯一致版 (AddHours -4)
# =============================================================================
...
$Form.Add_FormClosing({ param($sender, $e); if ($Script:IsDirty) { if ([System.Windows.Forms.MessageBox]::Show("設定未儲存，確定要離開？", "警告", "YesNo") -eq "No") { $e.Cancel = $true; return } }; $LastHash = ""; if (Test-Path $HashFile) { $LastHash = Get-Content $HashFile -Raw }; $CurrentHash = Get-CurrentScriptsHash; if ($CurrentHash -ne $LastHash) { if ([System.Windows.Forms.MessageBox]::Show("偵測到腳本核心已變更 (與上次發布不同)，是否同步至 GitHub？", "版本控制", "YesNo") -eq "Yes") { Start-Process powershell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PublishScript`"" } } })
...
# ... (TabResin, TabTools, TabLogs 保持不變) ...
# (請複製 V7.16 的剩餘代碼)
