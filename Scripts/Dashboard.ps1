...
$cbRConfig.Add_DropDown({ $cbRConfig.Items.Clear(); $real = $Global:ConfigList | Where-Object { $_ -ne "PAUSE" }; $cbRConfig.Items.AddRange($real) })
$cbRConfig.Add_SelectedIndexChanged({ $sel = $cbRConfig.Text; if ($Global:ResinData.ContainsKey($sel)) { $dat = $Global:ResinData.$sel; if ($dat.TaskType -eq "Stygian") { $rbStygian.Checked = $true } else { $rbDomain.Checked = $true }; if ($dat.ResinMode -eq "Count") { $rbCount.Checked = $true } else { $rbAll.Checked = $true }; $numOrig.Value = if ($dat.Counts.Original) { $dat.Counts.Original } else { 0 }; $numCond.Value = if ($dat.Counts.Condensed) { $dat.Counts.Condensed } else { 0 }; $numTran.Value = if ($dat.Counts.Transient) { $dat.Counts.Transient } else { 0 }; $numFrag.Value = if ($dat.Counts.Fragile) { $dat.Counts.Fragile } else { 0 }; $lstPrio.Items.Clear(); if ($dat.Priority) { $lstPrio.Items.AddRange($dat.Priority) } else { $lstPrio.Items.AddRange(@("浓缩树脂", "原粹树脂", "须臾树脂", "脆弱树脂")) } } else { $rbDomain.Checked = $true; $rbAll.Checked = $true; $numOrig.Value = 0; $numCond.Value = 0; $numTran.Value = 0; $numFrag.Value = 0; $lstPrio.Items.Clear(); $lstPrio.Items.AddRange(@("浓缩树脂", "原粹树脂", "须臾树脂", "脆弱树脂")) } })
$btnUp.Add_Click({ $idx = $lstPrio.SelectedIndex; if ($idx -gt 0) { $item = $lstPrio.SelectedItem; $lstPrio.Items.RemoveAt($idx); $lstPrio.Items.Insert($idx - 1, $item); $lstPrio.SelectedIndex = $idx - 1 } })
$btnDown.Add_Click({ $idx = $lstPrio.SelectedIndex; if ($idx -ge 0 -and $idx -lt $lstPrio.Items.Count - 1) { $item = $lstPrio.SelectedItem; $lstPrio.Items.RemoveAt($idx); $lstPrio.Items.Insert($idx + 1, $item); $lstPrio.SelectedIndex = $idx + 1 } })
$btnRSave.Add_Click({ $sel = $cbRConfig.Text; if (-not $sel) { [System.Windows.Forms.MessageBox]::Show("請先選擇配置組！"); return }; $prioList = @(); foreach ($i in $lstPrio.Items) { $prioList += $i }; $newData = @{ TaskType = if ($rbStygian.Checked) { "Stygian" } else { "Domain" }; ResinMode = if ($rbCount.Checked) { "Count" } else { "All" }; Priority = $prioList; Counts = @{ Original=$numOrig.Value; Condensed=$numCond.Value; Transient=$numTran.Value; Fragile=$numFrag.Value } }; $Global:ResinData.$sel = $newData; $Global:ResinData | ConvertTo-Json -Depth 5 | Set-Content $ResinConf -Encoding UTF8; [System.Windows.Forms.MessageBox]::Show("策略 [$sel] 已儲存！") })
...
$lblPath = New-Object System.Windows.Forms.Label; $lblPath.AutoSize=$true; $lblPath.Font=$MainFont; $lblPath.ForeColor="Gray"; $lblPath.Text = "目前遊戲路徑: 載入中..."; $flpTools.Controls.Add($lblPath)
function Update-PathLabel { $path = "尚未設定"; if ($Global:GenshinPath) { $path = $Global:GenshinPath }; $lblPath.Text = "目前遊戲路徑: $path" }
Add-ToolBtn "📂 設定原神遊戲路徑 (自動/手動)" "LightYellow" { $FoundPath = Auto-Detect-GenshinPath; $UseAuto = $false; if ($FoundPath) { $res = [System.Windows.Forms.MessageBox]::Show("✅ 自動偵測成功！`n`n找到路徑：`n$FoundPath`n`n是否直接使用此路徑？", "偵測結果", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Information); if ($res -eq "Yes") { $Global:GenshinPath = $FoundPath; $UseAuto = $true } } else { [System.Windows.Forms.MessageBox]::Show("❌ 自動偵測失敗。`n`n無法在標準安裝位置找到原神。`n請在接下來的視窗中手動選擇 'Genshin Impact Game' 資料夾。", "偵測結果", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning) }; if (-not $UseAuto) { $f = New-Object System.Windows.Forms.FolderBrowserDialog; $f.Description = "請選擇包含 YuanShen.exe / GenshinImpact.exe 的資料夾"; if ($f.ShowDialog() -eq "OK") { $Global:GenshinPath = $f.SelectedPath; $UseAuto = $true } }; if ($UseAuto) { $envData = @{ GenshinPath = $Global:GenshinPath }; $envData | ConvertTo-Json | Set-Content "$ConfigsDir\EnvConfig.json" -Encoding UTF8; [System.Windows.Forms.MessageBox]::Show("路徑已儲存！", "設定完成"); Update-PathLabel } }
Add-ToolBtn "[COPY] 複製配置組" "LightBlue" { $sel = Show-ConfigSelectorGUI ""; if($sel) { $sel = ($sel -split ",")[0]; $newName = [Microsoft.VisualBasic.Interaction]::InputBox("請輸入新配置組名稱:", "複製配置", "$sel-Copy"); if (-not [string]::IsNullOrWhiteSpace($newName)) { $src = Join-Path $BetterGI_UserDir "$sel.json"; $dst = Join-Path $BetterGI_UserDir "$newName.json"; if (Test-Path $src) { Copy-Item $src $dst -Force; $j = Get-Content $dst -Raw -Enc UTF8 | ConvertFrom-Json; $j.Name = $newName; $j | ConvertTo-Json -Depth 20 | Set-Content $dst -Enc UTF8; [System.Windows.Forms.MessageBox]::Show("已複製為: $newName"); Load-BetterGIConfigs } } } }
