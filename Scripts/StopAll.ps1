# ============================================================
# AutoTask 強制停止腳本 V3.0 (整合 GentleStop 登出邏輯)
# ============================================================
...
# 2. [最優先] 停止控制腳本 (Monitor/Master)
# 這是為了防止我們在清理過程中，Monitor 又雞婆地把程式開起來
...
# 3. [次優先] 強制清理業務應用程式 (BetterGI / 原神)
# 先把吃資源的程式殺掉，確保登出時不會卡住
...
# 4. [核心整合] 強制登出 Remote 帳戶 (引用 GentleStop 邏輯)
# 這是為了清除 RDP Session，避免下次連線黑屏或資源鎖定
Write-Host "`n[3/5] 正在清理遠端工作階段 (Remote Session)..." -ForegroundColor Cyan
$SessionOutput = qwinsta | Select-String "\bRemote\b"
if ($SessionOutput) {
    # 解析 Session ID
    ...
    if ($SessionID) {
        ...
        # 快速確認
        Start-Sleep 1
        if (-not (qwinsta | Select-String "\bRemote\b")) {
            Write-Host "    - Remote 已成功登出。" -ForegroundColor Green
        } else {
            Write-Host "    - 登出指令已發送 (後台處理中)。" -ForegroundColor Yellow
        }
    }
} else {
