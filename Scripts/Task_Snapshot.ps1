<#
.SYNOPSIS
    AutoTask Snapshot Tool V2.9
    用於打包所有腳本、設定檔與最近日誌，供 AI 進行除錯分析。
    V2.9 新增: 將 AutoTask_Core_Analysis.md 強制整合進 All_Source_Combined.txt。
    V2.8 新增: 全設定檔合併機制 (All_Configs_Combined.txt)。
    V2.7 新增: 自動匯出 Windows 排程器 (Task Scheduler) 設定檔 XML。
#>

$SnapshotVersion = "V2.9"
...
# 核心分析文件路徑 (優先讀取 Scripts 目錄，其次根目錄)
...
$SourceHeader = "=== [ AutoTask Full Source Code Dump ] ===`r`n`r`n" +
                ...
                "此檔案包含 AutoTask_Core_Analysis.md (核心邏輯)、所有 .bat 與 .ps1 的完整內容。`r`n" +
                "請 AI 以此檔案內容為最高準則進行分析。`r`n`r`n"
Set-Content -Path $CombinedSourceFile -Value $SourceHeader -Encoding UTF8

# [V2.9 新增] 優先寫入核心邏輯文件
if (Test-Path $CoreDocPath) {
    Write-Host "    已整合核心邏輯文件: $(Split-Path $CoreDocPath -Leaf)" -ForegroundColor Yellow
    $DocBlock = "`r`n######################################################################`r`n" +
                "FILE: AutoTask_Core_Analysis.md`r`n" +
                "PATH: $CoreDocPath`r`n" +
                "TYPE: Core Logic Documentation (SSOT)`r`n" +
                "######################################################################`r`n`r`n"
    Add-Content -Path $CombinedSourceFile -Value $DocBlock -Encoding UTF8
    
    $DocContent = Get-Content -Path $CoreDocPath -Raw
    Add-Content -Path $CombinedSourceFile -Value $DocContent -Encoding UTF8
} else {
    Write-Warning "    未找到 AutoTask_Core_Analysis.md，建議將文件放入 Scripts 目錄！"
}
...
foreach ($File in $SourceFiles) {
    # 避免重複寫入分析文件 (如果它剛好在 Scripts 下且副檔名不同 - 雖然這裡是 .md 不會被 .ps1 filter 抓到，但防呆)
